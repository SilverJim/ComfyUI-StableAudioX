            ...
            try:
                ...
                class DummyTokenizer:
                    def __call__(self, texts, **kwargs):
                        ...
                        return {"input_ids": torch.zeros(len(texts), 128, dtype=torch.long, device=device),
                               "attention_mask": torch.ones(len(texts), 128, dtype=torch.bool, device=device)}
                ...
                class DummyT5Model:
                    ...
                    def __call__(self, **kwargs):
                        ...
                        device = kwargs["input_ids"].device  # Use same device as input
                        return {"last_hidden_state": torch.zeros(batch_size, seq_len, self.model_dim, device=device)}
                    def to(self, device):
                        self.device = device
                        return self
                    def eval(self):
                ...
                self.model = DummyT5Model(self.T5_MODEL_DIMS[self.t5_model_name])
