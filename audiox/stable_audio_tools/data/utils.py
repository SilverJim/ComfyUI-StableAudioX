...
try:
    ...
    print("AudioX: decord available for video processing")
except ImportError:
    DECORD_AVAILABLE = False
    print("AudioX: decord not available - video processing will use fallback methods")
...
def read_video(filepath, seek_time=0., duration=-1, target_fps=2):
    ...
    # Check if decord is available for video processing
    if not DECORD_AVAILABLE:
        print(f"AudioX: decord not available - cannot process video file {filepath}")
        print("AudioX: Install decord with: pip install decord")
        print("AudioX: Returning empty video tensor")
        target_frames = int(duration * target_fps) if duration > 0 else 30  # Default 30 frames
    ...
    try:
        ...
        print(f"AudioX: Error reading video {filepath}: {e}")
        print("AudioX: Returning empty video tensor")
        target_frames = int(duration * target_fps) if duration > 0 else 30
        return torch.zeros((target_frames, 3, 224, 224))
