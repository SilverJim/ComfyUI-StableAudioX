...
def validate_and_normalize_device(device):
    """
    Validate and normalize device specification.

    Args:
        device: Device specification (string, torch.device, or None)
...
def ensure_tensor_device(tensor, target_device):
    ...
    target_device = validate_and_normalize_device(target_device)

    if hasattr(tensor, 'device') and str(tensor.device) != target_device:
        print(f"AudioX: Moving tensor from {tensor.device} to {target_device}")
...
def prepare_video_conditioning(video_tensor: torch.Tensor, text_prompt: str, model_config: Dict, device: str = None) -> Dict:
    """
    ...
    Args:
        ...
        device: Device to move tensors to (auto-detects if None)

    Returns:
        Conditioning dictionary
    """
    # Validate and normalize device
    device = validate_and_normalize_device(device)

    print(f"AudioX: Preparing video conditioning on device: {device}")
    print(f"AudioX: Input video tensor shape: {video_tensor.shape}, device: {video_tensor.device}")

    # Move video tensor to the correct device first
    video_tensor = ensure_tensor_device(video_tensor, device)
    ...
    print(f"AudioX: Final video conditioning tensor shape: {final_video_tensor.shape}, device: {final_video_tensor.device}")
...
def create_minimal_conditioning(text_prompt: str, duration_seconds: int, device: str = None) -> Dict:
    """
    ...
    Args:
        ...
        device: Device to create tensors on (auto-detects if None)
    ...
    print(f"AudioX: Creating minimal conditioning on device: {device}")
...
def create_video_only_conditioning(video_tensor: torch.Tensor, text_prompt: str,
                                 duration_seconds: int, device: str = None) -> Dict:
    """
    ...
    print(f"AudioX: Creating video-only conditioning for {duration_seconds}s generation on device: {device}")

    # Prepare video conditioning with proper frame standardization
    video_conditioning = prepare_video_conditioning(video_tensor, text_prompt, {}, device)
...
def create_enhanced_video_conditioning(video_tensor: torch.Tensor, text_prompt: str,
                                     duration_seconds: int, device: str = None,
    ...
    print(f"AudioX: Creating enhanced video conditioning on device: {device} (text_weight={text_weight}, video_weight={video_weight})")
    ...
    video_conditioning = prepare_video_conditioning(video_tensor, enhanced_text, {}, device)
...
def create_multi_aspect_conditioning(prompt: str, duration_seconds: int, device: str = None,
    ...
    print(f"AudioX: Creating multi-aspect conditioning on device: {device}")
...
def create_empty_conditioning_values(device: str = None, duration_seconds: int = 10) -> Dict:
    """
    ...
    print(f"AudioX: Creating empty conditioning values on device: {device}")
